...
def weight_stationary_total_latency(batch_size=1):
    dram_latency = DRAM_ACC_LATENCY

    # Calculate latency for 5 convolutional layers
    conv_layer_latencies = [0] * 5
    for i in range(5):
        conv_layer_latencies[i] = calc_weight_stationary_conv_latency(**conv_layer_params[i], batch_size=batch_size)
...
def calc_weight_stationary_conv_latency(
    ...
    return calc_weight_stationary_matrix_mult_latency(weight_rows=num_filter, input_rows=num_channel*filter_width*filter_height, input_cols=input_width*input_height*batch_size)


def calc_weight_stationary_matrix_mult_latency(weight_rows=32, input_rows=27, input_cols=1024):
