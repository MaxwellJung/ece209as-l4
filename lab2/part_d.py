...
def calc_weight_stationary_matrix_mult_latency(weight_rows=32, input_rows=27, input_cols=1024, dpu_size=DPU_SIZE):
    ...
    pipeline_latency = max(ACTIVATION_SRAM_LOAD_LATENCY, COMPUTE_LATENCY, ACTIVATION_SRAM_WRITE_LATENCY)
    latency_per_weight_update = WEIGHT_SRAM_LOAD_LATENCY + \
        input_buf_update_count_per_weight_update*pipeline_latency + \
        pipeline_latency
