...
class StraightThroughEstimator(torch.autograd.Function):
    ...
    def forward(ctx, input: Tensor):
        ...
        # Map f32 to fake i8
        q = (input - weight_min_f32)/(weight_max_f32-weight_min_f32)*(i8.max-i8.min) + i8.min
        q = torch.clamp(torch.round(q), min=-128, max=127)
...
for epoch in range(num_epochs):
    ...
    if acc > best_acc:
        ...
        best_acc = acc
        torch.save(model.state_dict(), './lab1/part2a.pth')

print(f'Best test accuracy: {best_acc*100:.2f}%')
print('Training completed! Model saved as lab1/part2a.pth')
