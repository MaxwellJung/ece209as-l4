...
weight_min_f32 = -32/32
weight_max_f32 = 31/32
i6_min = -32
i6_max = 31
...
class StraightThroughEstimator(torch.autograd.Function):
    ...
    def forward(ctx, input: Tensor):
        ...
        # Map f32 to fake i6
        q = (input - weight_min_f32)/(weight_max_f32-weight_min_f32)*(i6_max-i6_min) + i6_min
        q = torch.clamp(torch.round(q), min=-32, max=31)
        # Map [-32,31] to range [-32/32,31/32]
        q = q/(2**5)
        return q
...
model.load_state_dict(torch.load('./lab1/part2-default.pth', map_location=device))
...
for epoch in range(num_epochs):
    ...
    if acc > best_acc:
        ...
        best_acc = acc
        torch.save(model.state_dict(), './lab1/part2b.pth')

print(f'Best test accuracy: {best_acc*100:.2f}%')
print('Training completed! Model saved as lab1/part2b.pth')
